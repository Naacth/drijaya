<?= $this->extend('layout/main') ?>
<?= $this->section('content') ?>
<div class="d-flex justify-content-between align-items-center mb-4 animate-in">
    <div>
        <a href="<?= site_url('pengadaan-barang') ?>" class="text-decoration-none text-muted mb-2 d-inline-block small"><i class="bi bi-arrow-left me-1"></i> Kembali</a>
        <h4 class="mb-1 fw-bold"><?= $title ?></h4>
    </div>
</div>
<div class="data-card animate-in" style="animation-delay: 0.1s;">
    <div class="card-header"><h6><i class="bi bi-pencil me-2"></i>Edit Pengadaan Barang</h6></div>
    <div class="card-body p-4">
        <form action="<?= site_url('pengadaan-barang/update/' . $header['id']) ?>" method="post">
            <div class="row g-3">
                <div class="col-md-6">
                    <label class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" name="tanggal" class="form-control" value="<?= $header['tanggal'] ?>" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Status</label>
                    <select name="status" class="form-select">
                        <option value="draft" <?= $header['status'] == 'draft' ? 'selected' : '' ?>>Draft</option>
                        <option value="diajukan" <?= $header['status'] == 'diajukan' ? 'selected' : '' ?>>Diajukan</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Nama Barang <span class="text-danger">*</span></label>
                    <input type="text" name="nama_barang" class="form-control" value="<?= esc($header['nama_barang']) ?>" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Jumlah <span class="text-danger">*</span></label>
                    <input type="number" name="jumlah" class="form-control" value="<?= $header['jumlah'] ?>" min="1" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Satuan</label>
                    <input type="text" name="satuan" class="form-control" value="<?= esc($header['satuan']) ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Estimasi Harga (Rp)</label>
                    <input type="number" name="estimasi_harga" class="form-control" value="<?= $header['estimasi_harga'] ?>" min="0" step="1000">
                </div>
                <div class="col-md-12">
                    <label class="form-label">Alasan Pengadaan <span class="text-danger">*</span></label>
                    <textarea name="alasan" class="form-control" rows="3" required><?= esc($header['alasan']) ?></textarea>
                </div>
            </div>
            <div class="mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i> Perbarui</button>
                <a href="<?= site_url('pengadaan-barang') ?>" class="btn btn-outline-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
<?= $this->endSection() ?>
