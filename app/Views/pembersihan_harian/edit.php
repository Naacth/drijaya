...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <form action="<?= site_url('pembersihan-harian/update/' . $header['id']) ?>" method="post">
                <div class="card shadow-sm border-0 overflow-hidden">
                    <div class="card-header bg-white py-3 border-bottom">
                        <h5 class="mb-0">Edit Form Pembersihan Freezer & Chiller Harian</h5>
                    </div>
                    <div class="card-body">
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="unit_type" id="unit_type" class="form-select" onchange="toggleItems()">
                                    <option value="chiller" <?= $header['unit_type'] == 'chiller' ? 'selected' : '' ?>>Chiller</option>
                                    <option value="freezer" <?= $header['unit_type'] == 'freezer' ? 'selected' : '' ?>>Freezer</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div id="chiller-items" style="<?= $header['unit_type'] == 'chiller' ? '' : 'display:none;' ?>">
                            ...
                            <div class="row g-2">
                                <?php foreach(['Rak', 'Kontainer', 'Lampu', 'Langit-langit', 'Lantai', 'Dinding', 'Bodi Luar', 'Gagang Pintu'] as $item): 
                                    ...
                                    $checked = isset($area[$key]) && $area[$key] == '1' ? 'checked' : '';
                                ?>
                                <div class="col-md-4">
                                    <div class="form-check p-2 border rounded">
                                        <input class="form-check-input ms-0 me-2" type="checkbox" name="area[<?= $key ?>]" value="1" <?= $checked ?>>
                        ...
                        <div id="freezer-items" style="<?= $header['unit_type'] == 'freezer' ? '' : 'display:none;' ?>">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="nama_petugas" class="form-control" value="<?= $header['nama_petugas'] ?>" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Nama Verifikator</label>
                                <input type="text" name="nama_verifikator" class="form-control" value="<?= $header['nama_verifikator'] ?>" required>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer bg-light text-end py-3">
                        <a href="<?= site_url('pembersihan-harian') ?>" class="btn btn-link link-secondary">Batal</a>
                        <button type="submit" class="btn btn-primary px-5 shadow-sm">Perbarui</button>
                    </div>
                </div>
            </form>
        </div>
