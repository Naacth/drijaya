...
echo "Tables dropped. Creating tables...\n";

// 1. Uji Cita Rasa
$db->query("CREATE TABLE `uji_cita_rasa` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, tanggal DATE, nama_petugas VARCHAR(255), created_by INT UNSIGNED, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)");
$db->query("CREATE TABLE `uji_cita_rasa_items` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, uji_cita_rasa_id INT UNSIGNED, nama_masakan VARCHAR(255), gramasi_standar VARCHAR(100), gramasi_real VARCHAR(100), masalah TEXT, penyelesaian TEXT, verifikator_chef VARCHAR(255), verifikator_gizi VARCHAR(255), CONSTRAINT fk_uji_cita_rasa FOREIGN KEY (uji_cita_rasa_id) REFERENCES uji_cita_rasa(id) ON DELETE CASCADE)");

// 2. Pemeriksaan Sampel
$db->query("CREATE TABLE `pemeriksaan_sampel` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, tanggal DATE, jam_matang TIME, jenis_produk VARCHAR(255), bahaya_fisik VARCHAR(100), bahaya_biologi VARCHAR(100), jam_penarikan TIME, tindak_lanjut TEXT, jam_ambil_sampel TIME, tgl_pemusnahan DATE, nama_petugas VARCHAR(255), created_by INT UNSIGNED, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)");

// 3. Makanan Lebih
$db->query("CREATE TABLE `makanan_lebih` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, tanggal DATE, nama_petugas VARCHAR(255), created_by INT UNSIGNED, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)");
$db->query("CREATE TABLE `makanan_lebih_items` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, makanan_lebih_id INT UNSIGNED, menu_makanan VARCHAR(255), porsi_awal INT, porsi_sisa INT, penanganan VARCHAR(100), jam_penanganan TIME, keterangan TEXT, CONSTRAINT fk_makanan_lebih FOREIGN KEY (makanan_lebih_id) REFERENCES makanan_lebih(id) ON DELETE CASCADE)");

// 4. Serah Terima Bahan
$db->query("CREATE TABLE `serah_terima_bahan` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, tanggal DATE, shift VARCHAR(50), nama_petugas VARCHAR(255), created_by INT UNSIGNED, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)");
$db->query("CREATE TABLE `serah_terima_bahan_items` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, serah_terima_bahan_id INT UNSIGNED, nama_bahan VARCHAR(255), jumlah_awal VARCHAR(100), jumlah_akhir VARCHAR(100), kondisi VARCHAR(100), keterangan TEXT, CONSTRAINT fk_serah_terima FOREIGN KEY (serah_terima_bahan_id) REFERENCES serah_terima_bahan(id) ON DELETE CASCADE)");

// 5. Monitoring Suhu Masak
$db->query("CREATE TABLE `monitoring_suhu_masak` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, tanggal DATE, nama_petugas VARCHAR(255), created_by INT UNSIGNED, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)");
$db->query("CREATE TABLE `monitoring_suhu_masak_items` (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, monitoring_suhu_masak_id INT UNSIGNED, menu_masakan VARCHAR(255), jam_masak TIME, suhu_inti VARCHAR(50), jam_selesai TIME, keterangan TEXT, CONSTRAINT fk_suhu_masak FOREIGN KEY (monitoring_suhu_masak_id) REFERENCES monitoring_suhu_masak(id) ON DELETE CASCADE)");
...
echo "Tables created. Updating migration records...\n";
$time = time();
$versions = ['2026-03-28-000001','2026-03-28-000002','2026-03-28-000003','2026-03-28-000004','2026-03-28-000005','2026-03-28-000006','2026-03-28-000007','2026-03-28-000008','2026-03-28-000009','2026-03-28-000010'];
foreach ($versions as $v) {
    $db->query("DELETE FROM migrations WHERE version = '$v'");
    $db->query("INSERT INTO migrations (version, `class`, `group`, `namespace`, `time`, `batch`) VALUES ('$v', 'App\\\\Database\\\\Migrations\\\\ManualFix', 'default', 'App', $time, 4)");
}
